...
namespace Lockstep.Game
{
    public interface IFrameBuffer
    {
        ...
        void PushServerFrame(ServerFrame frame, bool isNeedDebugCheck = true);
    ...
    public class FrameBuffer : IFrameBuffer
    {
        public class PredictCountHelper
        {
            ...
            private float _targetPreSendTick = 0;
            ...
            public void DoUpdate(float deltaTime)
            {
                ...
                if (_timer >= _checkInterval)
                {
                    ...
                    if (!hasMissTick)
                    {
                        var preSend = _cmdBuffer.MaxPing * 1.0f / NetworkDefine.UPDATE_DELTATIME;
                        _targetPreSendTick = _targetPreSendTick * _oldPercent + preSend * (1 - _oldPercent);

                        int targetPreSendTick = LMath.Clamp((int)System.Math.Ceiling(_targetPreSendTick), 1, 60);
#if UNITY_EDITOR
                        if (targetPreSendTick != _simulatorService.PreSendInputCount)
                        {
                            Debug.LogWarning(
                                ...
                                $"PING: min:{_cmdBuffer.MinPing} max:{_cmdBuffer.MaxPing} avg:{_cmdBuffer.PingVal}");
                        }
#endif
        ...
        private int _maxServerOverFrameCount = 0;
        ...
        private long __minPing = Int64.MaxValue;
        public long MinPing { get { return __minPing; } private set { __minPing = value; } }
        private long __maxPing = Int64.MinValue;
        public long MaxPing { get { return __maxPing; } private set { __maxPing = value; } }
        ...
        public void OnPlayerPing(Msg_G2C_PlayerPing msg)
        {
            ...
            {
                MinPing = ping;
                _guessServerStartTimestamp = (LTime.realtimeSinceStartupMS - msg.timeSinceServerStart) - MinPing / 2;
            }
        ...
        public void PushServerFrames(ServerFrame[] frames, bool isNeedDebugCheck = true)
        {
            ...
            {
                PushServerFrame(frames[i], isNeedDebugCheck);
            }
        }

        public void PushServerFrame(ServerFrame frame, bool isNeedDebugCheck = true)
        {
        ...
        private void UpdatePingVal(float deltaTime)
        {
            ...
            if (_pingTimer > 0.5f)
            {
                ...
                if (MinPing < _historyMinPing && _simulatorService._gameStartTimestampMs != -1)
                {
                    _historyMinPing = MinPing;
#if UNITY_EDITOR
                    Debug.LogWarning(
